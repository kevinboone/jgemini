...
The settings `ui.control_font` and `ui.user_font` control the appearance of the
user interface elements other than the document display. Broadly, the "user"
font applies to text entry controls, while the "control" font applies to
everything else (menus, buttons...). I'm sorry about this, but the font
configuration here is Java-like ("name size"), while the document display
styles are CSS-like ("size name"). In addition, while the document font sizes
...
other than links and headers, I often see people using markdown-style emphasis
...
simply carries out no certificate checks. Is this a security problem?  Sure. If
...
The Gemini protocol only supports one (very simple) method for supplying user
input. Those sites that handle user input expects a single line of user input
appended to the request URL after a '?' character. If the expected input is
