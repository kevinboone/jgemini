...
For more about how Gemini deals with authentication and identity, see
the page on [identity](identity.md).

As with everything else in JGemini, you can control which certificate to send
by editing the [configuration file](config_file.md). However, in most cases you
won't need to: it's usually easier to manage identity using the [Set
identity](set_identity_dialog.md) dialog.  For first steps using JGemini's
built-in identity manager, please see [Getting started with
identity](getting_started_with_identity.md).
...
Each client certificate must be placed in a Java keystore, along with the
...
enter the password. Then you can assign this identity to remote
hosts.

### Editing the configuration file

An entry in the configuration file that maps a particular hostname to an
identity looks this:
...
to that host. The identity can be any string. To map the identity string to
...
What if you want to send a client certificate to every host except
