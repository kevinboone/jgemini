...
hassle of hunting around for the file. In addition, using the built-in editor
allows JGemini to keep its internal cache in sync with the file contents.
Editing the bookmarks file with a text editor, with JGemini running, could
have odd results.

When you show bookmarks using Bookmarks|Show all, JGemini just renders the bookmark
file as if it were any other Gemtext document.

JGemini also shows a portion of the bookmarks file in the bookmarks menu. 
By default it shows the first ten items, but you can change this by modifying
the setting `bookmark.max.menu` in the [configuration file](config_file.md).
The first ten items are assigned the keyboard accelerators '0' to '9'.
